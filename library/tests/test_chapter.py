...
from library.chapter import Chapter, ChapterMetadata, MediaResource, MarkdownStorage, HtmlStorage, MhtmlStorage, Base64Storage
...
def sample_chapter():
    ...
    media = [
        MediaResource.from_bytes("test.png", b"fake-png-data", "image/png")
    ]
    return Chapter(metadata=metadata, content=content, content_type="text/html", media=media)

@pytest.mark.parametrize("storage_class, ext", [
    (MarkdownStorage, ".md"),
    (HtmlStorage, ".html"),
    (MhtmlStorage, ".mhtml"),
    (Base64Storage, ".json"),
...
def test_storage_roundtrip(storage_class, ext, sample_chapter, tmp_path):
    ...
    if storage_class in [MarkdownStorage, HtmlStorage]:
        assets_dir = tmp_path / f"test_chapter.assets"
