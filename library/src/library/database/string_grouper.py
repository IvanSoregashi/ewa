"""from https://github.com/Bergvca/string_grouper version 0.7.1
updated dependencies and logger, other things
"""
...
# significance
# - Option value to use to split dataset(s) into roughly equal-sized blocks
DEFAULT_N_BLOCKS: tuple[int, int] | None = None
...
def compute_pairwise_similarities(string_series_1: pd.Series, string_series_2: pd.Series, **kwargs) -> pd.Series:
    """
...
def group_similar_strings(
    strings_to_group: pd.Series, string_ids: pd.Series | None = None, **kwargs
    ...
    """
    sg = StringGrouper(strings_to_group, master_id=string_ids, **kwargs)
    return sg.fit().get_groups()
...
def match_most_similar(
    ...
    **kwargs,
    ...
    sg = StringGrouper(master, duplicates=duplicates, master_id=master_id, duplicates_id=duplicates_id, **kwargs)
    return sg.fit().get_groups()


def match_strings(
    master: pd.Series,
    duplicates: pd.Series | None = None,
    master_id: pd.Series | None = None,
    duplicates_id: pd.Series | None = None,
    **kwargs,
) -> pd.DataFrame:
...
class StringGrouperConfig(NamedTuple):
    ...
    max_n_matches: int | None = DEFAULT_MAX_N_MATCHES  # None
...
class StringGrouper(object):
    def __init__(
        ...
        **kwargs,
        ...
        self._duplicates: pd.Series | None = None
        self._master_id: pd.Series | None = None
        self._duplicates_id: pd.Series | None = None
    ...
    def fit(self):
        ...
        b_left = max(
            1, round(len(self._left_Series) / 1e6)
        )  # arbitrary, big enough not to split both left and right often
    ...
    @validate_is_fit
    def get_matches(self, ignore_index: bool | None = None, include_zeroes: bool | None = None) -> pd.DataFrame:
        """
        ...
        if self._config.min_similarity > 0 or not include_zeroes:
            ...
            matches_list = (
                ...
                else pd.concat([self._matches_list, non_matches_list], axis=0, ignore_index=True)
            )
        ...
        if self._master_id is None:
            ...
            left_side_id, right_side_id = get_both_sides(
                self._master_id, self._duplicates_id, matches_list, (DEFAULT_ID_NAME, DEFAULT_ID_NAME), drop_index=True
            )
            return pd.concat(
                [
                    prefix_column_names(left_side, LEFT_PREFIX),
    ...
    def compute_pairwise_similarities(
        self, string_series_1: pd.Series, string_series_2: pd.Series, **kwargs
    ) -> pd.Series:
        """
    ...
    def remove_match(self, master_side: str, dupe_side: str) -> "StringGrouper":
        ...
        self._matches_list = self._matches_list[
            ~((self._matches_list.master_side.isin(master_indices)) & (self._matches_list.dupe_side.isin(dupe_indices)))
        ]
        return self
    ...
    def _build_matches(
        self, master_matrix: csr_matrix, duplicate_matrix: csr_matrix, n_blocks: tuple[int, int]
    ) -> csr_matrix:
        ...
        def define_chunks(length_to_split, n_chunks):
            ...
            chunk_len = int(np.ceil(length_to_split / n_chunks))
    ...
    def _get_matches_list(self, matches: csr_matrix) -> pd.DataFrame:
        """Returns a list of all the indices of matches"""
        r, c = matches.nonzero()
        d = matches.data
        return pd.DataFrame({"master_side": r.astype(np.int64), "dupe_side": c.astype(np.int64), "similarity": d})
    ...
    def _get_nearest_matches(self, ignore_index=False, replace_na=False) ->  pd.DataFrame | pd.Series:
        ...
        if self._master_id is not None:
            ...
            if (
                ...
                dupes_max_sim.loc[:, master_id_label] = dupes_max_sim.loc[:, master_id_label].astype(
                    self._master_id.dtype
                )
    ...
    def _deduplicate(self, ignore_index=False) ->  pd.DataFrame | pd.Series:
        ...
        n = len(self._master)
        graph = csr_matrix(
            (np.full(len(pairs), 1), (pairs.master_side.to_numpy(), pairs.dupe_side.to_numpy())), shape=(n, n)
        )
        ...
        group_of_master_index["group_rep"] = group_of_master_index.groupby("raw_group_id", sort=False)[
            "weight"
        ].transform(method)
        ...
        if isinstance(output, pd.DataFrame):
            output.rename(columns={col: f"{prefix}{col}" for col in output.columns if str(col) != label}, inplace=True)
        if self._master_id is not None:
    ...
    def _validate_group_rep_specs(self):
        ...
        if self._config.group_rep not in group_rep_options:
            raise Exception(f"Invalid option value for group_rep. The only permitted values are\n {group_rep_options}")
    ...
    def _cross_join(dupe_indices, master_indices, similarities) -> pd.DataFrame:
        x_join_index = pd.MultiIndex.from_product(
            [master_indices, dupe_indices, similarities], names=["master_side", "dupe_side", "similarity"]
        )
    ...
    def _is_series_of_strings(series_to_test: pd.Series) -> bool:
        if not isinstance(series_to_test, pd.Series):
            ...
            series_to_test.to_frame().apply(lambda row: row.map(lambda x: not isinstance(x, str))).squeeze(axis=1).any()
        ):
            return False
        return True
