...
class Metadata(BaseXmlModel, tag="metadata", ns=NamespacePrefix.OPF, nsmap=OPF_NSMAP):
    language: DCElement = element(tag=DCMetadataType.LANGUAGE)
    identifier: DCElement = element(tag=DCMetadataType.IDENTIFIER)
    title: DCElement = element(tag=DCMetadataType.TITLE)
    creator: DCElement = element(tag=DCMetadataType.CREATOR)
    description: DCElement = element(tag=DCMetadataType.DESCRIPTION)
